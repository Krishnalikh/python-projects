...
def create_oval_button(text, command, x, y, time_offset):
    ...
    # Create a label for digital time below the button
    digital_clock_label = tk.Label(root, text="", font=("Helvetica", 14, "bold"), fg="white", bg="black")
    digital_clock_label.place(x=x+25, y=y+55)
...
def show_uk_time():
    show_time_zone("UK Time", 3600, "BST")  # UTC+1 during British Summer Time

def show_us_time():
    show_time_zone("US Time", -14400, "EDT")  # UTC-4 during Eastern Daylight Time
...
root.geometry("1300x800")  # Adjusted height and width to fit the new button
...
canada_flag_frames = load_gif_frames('canada_flag.gif', 50, 30)  # Load Canada flag GIF
france_flag_frames = load_gif_frames('france_flag.gif', 50, 30)  # Load France flag GIF
...
animate_gif(animation_canvas, indian_flag_frames, 50, 50)  # Above the "INDIAN TIME" button
animate_gif(animation_canvas, australia_flag_frames, 230, 50)  # Above the "AUSTRALIA TIME" button
animate_gif(animation_canvas, uk_flag_frames, 410, 50)  # Above the "UK TIME" button
animate_gif(animation_canvas, us_flag_frames, 590, 50)  # Above the "US TIME" button
animate_gif(animation_canvas, japan_flag_frames, 770, 50)  # Above the "JAPAN TIME" button
animate_gif(animation_canvas, canada_flag_frames, 950, 50)  # Above the "CANADA TIME" button
animate_gif(animation_canvas, france_flag_frames, 1130, 50)  # Above the "FRANCE TIME" button

# Create the oval buttons with digital clocks
...
create_oval_button("CAN", show_canada_time, 950, 150, -14400)  # Add Canada Time button
create_oval_button("FRA", show_france_time, 1130, 150, 7200)  # Add France Time button

# Load and display the world map image
world_map_img = Image.open("world_map.jpg")
world_map_img = world_map_img.resize((900, 400), Image.LANCZOS)  # Resize to fit
world_map = ImageTk.PhotoImage(world_map_img)

# Place the world map on the screen
map_label = tk.Label(root, image=world_map, bg="black")
map_label.pack(pady=(100, 20))  # Place with some padding above and below
...
global_time_label.place(x=540, y=500)  # Place it in the center of the map
